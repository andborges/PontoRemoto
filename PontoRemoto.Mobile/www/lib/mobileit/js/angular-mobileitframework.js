...
angular.module("templates/loader.tpl", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("templates/loader.tpl",
	...
	"	<i></i>\n" +
	"	<div id=\"mblLoaderText\" ng-transclude></div>\n" +
	"</div>");
}]);
...
angular.module("templates/scrollerPullDown.tpl", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("templates/scrollerPullDown.tpl",
    "<div id='pullDown'>\n" +
	"	<span class='pullDownIcon'></span><i class='icon ion-refreshing'></i> <span class='pullDownLabel'>Puxe para atualizar</span>\n" +
	"</div>");
}]);

(function() {
	var directiveModules = angular.module('mobileit.directives', ['templates-main', 'ngCordova']); // [] -> create new module
	...
	directiveModules.factory('setup', function($rootScope, $location, $cordovaToast) {
		return {
					initialize: function(options) {
						...
						$rootScope.mbl.app.logoff = function() {
							...
							if ($rootScope.mbl.screen.snapper)
						...
						$rootScope.mbl.app.toast = function (message) {
							if (ripple()) {
						...
						document.addEventListener("backbutton", function (e) {
							...
							if ($location.path() != "" && $location.path() != $rootScope.mbl.options.firstPage) {
	...
	directiveModules.factory('api', function($rootScope, $http, crypt, $cordovaNetwork) {
		var factory = {};

		factory.post = function(url, data, params, successCallback, errorCallback) {
			if ($cordovaNetwork.isOffline()) {
				errorCallback("Sem rede disponível");
				return;
			}
			...
			var fullUrl = $rootScope.mbl.options.server + "/Api/" + $rootScope.mbl.options.apiversion + "/" + url + "?";
				fullUrl += ("hash=" + crypt.encrypt(guid()));
			...
			.error(function(data, status) {
				...
				if (status == 404)
					successCallback("Item não encontrado");
				else
					errorCallback(data.Message ? data.Message : "Erro inesperado");
			});
		}
		
		factory.get = function(url, querystring, params, successCallback, errorCallback) {
			if ($cordovaNetwork.isOffline()) {
				errorCallback("Sem rede disponível");
				return;
			}
		...
		return {
			...
			link: function($scope, element, attrs) {
				...
				$rootScope.mbl.page.addScroll = function(offset) {
					...
					var navbar = element.parents(".mbl-page").find(".bar-header");
					var navsubbar = element.parents(".mbl-page").find(".bar-subheader");
					var pullDown = element.parents(".mbl-page").find("#pullDown");
					var tabbar = $(".tabs");
					...
					if (navsubbar.length) {
						$rootScope.mbl.page.top += navsubbar.outerHeight();
					}
					...
					pullDown.offset({ top: $rootScope.mbl.page.top });
	...
	return new iScroll(id,
						{
							...
							onRefresh: function () {
								if (pullDownEl[0]) {
									pullDownEl.removeClass('refresh');
									pullDownEl.find('.pullDownLabel').html('Puxe para atualizar');
								}
							},
							onScrollMove: function (that, e) {
								if (pullDownEl[0]) {
									if (this.y > pullDownOffset && !pullDownEl.hasClass('refresh')) {
										pullDownEl.addClass('refresh');
										pullDownEl.find('.pullDownLabel').html('Solte para atualizar');
									} else if (this.y <= pullDownOffset && pullDownEl.hasClass('refresh')) {
										pullDownEl.removeClass('refresh');
										pullDownEl.find('.pullDownLabel').html('Puxe para atualizar');
									}
								}
							...
							onScrollEnd: function () {
								if (pullDownEl[0] && pullDownEl.hasClass('refresh')) {
...
function waitDomReady(callback) {
	setTimeout(function() {
		callback();
	}, 100);
}